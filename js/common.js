...
if($(".skladskaja-logistika #time").length > 0){
	var oneDate = new Date("12/26/2017 20:00:00");
...
$(function () {
	...
	$(window).bind('scroll', function () {
		if ($(window).scrollTop() > 800) {
			$('.headerbf').addClass('fixed');
			$('.headerbf').removeClass('normal');
		} else {
			$('.headerbf').removeClass('fixed');
			$('.headerbf').addClass('normal');
		}
	});
	...
	button.onclick = function() {
		if ( -1 !== container.className.indexOf( 'toggled' ) ) {
			...
			$(".site-navigation.toggled").on("click", "ul li a", function() {
				container.className = container.className.replace( ' toggled', '' );
			});
		}
	};
